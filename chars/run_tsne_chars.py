...
if __name__ == "__main__":
    ...
    X = np.load("data/chars-processed.npy")[:40]
    print('X.shape=',X.shape)
    y = np.load("data/chars-labels-processed.npy")[:40]
    ...
    for beta in betas:
        precomputed = sdtw_matrix(X, beta, n_jobs=2)
        train = precomputed.copy()
        # shift the distance to avoid negative values
        train -= train.min()
        tsne = TSNE(metric="precomputed")
        print("Default perplexity:", tsne.perplexity)
        tsne_data = TSNE(metric="precomputed").fit_transform(train)
