...
def gaussian_mixture(means, sds, values, gaps, T, masses, offset=True):
    x = []
    for m, sd, mass in zip(means, sds, masses):
...
if __name__ == "__main__":
    ...
    values = [5, 0, 3]
    gaps = [6, 12, 8]
    signal = gaussian_mixture(means, sds, values, gaps, 2 * T, masses=[mass, mass],
                              offset=True)
    x = signal[50: 50 + T]
